...
- one uniform prior for all 10 digits which is a uniform distribution over the persistence diagram spacei(\theta) = \sum_\limits{j = 1}^J \omega_j \mathcal{N}(\theta | \mu_j, \Sigma_j)$
    - J = 1, $\omega = 1$
    - mean vector $[.5, .5]^\top$
...
```{r, fig.dim=c(10,6)}
## ggplot function for confusion matrix heatmap
...
conf_matrix_nn_dropout <- nn_dropout_conf_mat$table
as.data.frame.matrix(conf_matrix_nn_dropout) %>%
...
conf_matrix_nn_ridge <- nn_ridge_conf_mat$table
as.data.frame.matrix(conf_matrix_nn_ridge) %>%
  mutate(true_label = fct_rev(rownames(.))) |>
  pivot_longer(
    cols = -true_label,
    names_to = "predicted_label",
...
## Confusion Matrix Heatmap - NN lasso
```{r, fig.dim=c(10,6)}
nn_lasso_conf_mat <- readRDS("nn_lasso_conf_mat")
# cm_ggplot(nn_ridge_conf_mat, type = "nn_ridge")
conf_matrix_nn_lasso <- nn_lasso_conf_mat$table
as.data.frame.matrix(conf_matrix_nn_lasso) %>%
  mutate(true_label = fct_rev(rownames(.))) |>
  pivot_longer(
    cols = -true_label,
    names_to = "predicted_label",
...
conf_matrix_mlogit <- mlogit_conf_mat$table
as.data.frame.matrix(conf_matrix_mlogit) %>%
  mutate(true_label = fct_rev(rownames(.))) |>
  pivot_longer(
    cols = -true_label,
    names_to = "predicted_label",
