...
:::{.incremental}
- **1988**: Creation of the **USPS database** (16×16 grayscale images of handwritten zip codes) for training the 1989 LeNet neural network.
- **Late 1980s**: National Institute of Standards and Technology's (NIST) **Special Databases** developed for Optical Character Recognition(OCR) research, derived from census forms and high-school student samples.
  - **SD-7** (1992): Test set of 58,646 images from 500 high-school students, later used in MNIST.
- **1992**: NIST/Census Bureau competition highlighted challenges in adapting models across datasets.
- **1994**: **MNIST** created, addressing biases in original NIST data
:::
...
- **Version Evolution**:
  - **QMNIST** (2019): Restored full 60k test set.
  - **EMNIST** (2017): Extended to include letters and digits.
  - **Fashion MNIST** (2017): Challenging alternative with fashion items.
:::


## Example digit {.smaller}
...
:::{.fragment}
-   Each pixel is given a gray-scale number ranging from 0 (white background) to 255 (black background)
-   We will binarize this to 0 and 1, changing it from gray-scale to black and white
:::
...
:::{.notes}
Binary filtration straight to persistence diagram, the images are cubical complexes
:::

## Project Objective

::: {.fragment}
**Goal:** Classify handwritten digits (pixel arrays) into their correct numerical labels.
:::
...
-   Captures shape-based features\
-   Simpler
-   Provides characteristics for some level of interpretability
:::
:::::
::::::
...
::: {.incremental}
-   **Shape of the data refers to**
    -   The digit "**1**" has no loops
    -   The digit "**0**" has one loop,
    -   The digit "**8**" has two loops
:::

::: {.fragment}
-   The goal is to summarize the pixels into features and aid machine learning models
:::
...
## Measuring Density Similarity

```{r, fig.dim=c(10,6)}
set.seed(7291009)
n_samples <- 5000

dist_names <- c("Dist A", "Dist B", "Dist C")
dist_colors <- setNames(brewer.pal(length(dist_names), "Set1"), dist_names)

# Generate and combine data
data <- tibble(
  `Dist A` = rnorm(n_samples, 0, .8),
  `Dist B` = rnorm(n_samples, 1, 1),
  `Dist C` = rnorm(n_samples, 5, 1)
) |>
  pivot_longer(everything(), names_to = "distribution", values_to = "value") |>
  mutate(distribution = factor(distribution, levels = dist_names))

create_w_plot <- function(data, dist1_name, dist2_name) {
  
  df_subset <- data |>
    filter(distribution %in% c(dist1_name, dist2_name))
  
  x1 <- df_subset |> filter(distribution == dist1_name) |> pull(value)
  x2 <- df_subset |> filter(distribution == dist2_name) |> pull(value)

  w_dist <- transport::wasserstein1d(x1, x2, p = 1)
  ...
  ribbon_data <- tibble(
    x = x_grid,
    ymin = pmin(ecdf1(x_grid), ecdf2(x_grid)),
    ymax = pmax(ecdf1(x_grid), ecdf2(x_grid))
  )

  ggplot(df_subset, aes(x = value, color = distribution)) +
    geom_ribbon(
      data = ribbon_data,
      aes(x = x, ymin = ymin, ymax = ymax),
      fill = "skyblue", alpha = 0.6, inherit.aes = FALSE
    ) +
    stat_ecdf(geom = "step", linewidth = 1.1) +
    scale_color_manual(values = dist_colors) + 
    labs(
      title = paste(dist1_name, "vs.", dist2_name),
      subtitle = bquote(W[1] == .(round(w_dist, 3))),
...
p_density <- ggplot(data, aes(x = value, fill = distribution)) +
  geom_density(alpha = 0.7, color = "white") +
  scale_fill_manual(values = dist_colors) + 
  labs(
    title = "Density of the Three Sampled Distributions",
    x = "Value", y = "Density"
  ) +
  theme_minimal(base_size = 14) +
...
```{r, fig.dim=c(10,6)}
w1 <- create_w_plot(data, "Dist A", "Dist B")
w2 <- create_w_plot(data, "Dist A", "Dist C")
w3 <- create_w_plot(data, "Dist B", "Dist C")
...
Wasserstein also known as Kantorovich-Rubinstein metric
:::
